...
- `ref`: The function gets a reference with a parametric mutability: that is,
  the reference is either mutable or immutable. You can think of `ref`
...
and destructor aren't invoked for a `read` argument.
...
The major difference between Rust and Mojo is that Mojo doesn't require a sigil
...
Technically, the `var` keyword doesn't guarantee that the received value is
...
- The caller **doesn't** use the `^` transfer sigil, in which case, Mojo copies
...
- The caller passes in a newly-created "owned" value, such as a value returned
  from a function. In this case, no variable owns the value and it's
