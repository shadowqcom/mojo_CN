...
- When working with types like
  ...
  [`Span`](/docs/std/memory/span/Span/) which are parameterized on the
...
can use to specify origin types. As the names suggest, the `ImmutOrigin` and
...
Or you can use the [`Origin`](/docs/std/builtin/type_aliases/Origin/)
...
- Static origin. The `StaticConstantOrigin` `comptime` value
...
- External origins. The external origins, `MutExternalOrigin` and
  `ImmutExternalOrigin` represent values that are not tracked by the lifetime
  checker, such as dynamically-allocated memory.
- Wildcard origins. The `ImmutAnyOrigin` and `MutAnyOrigin` `comptime` values
...
returns a
[`StringSlice`](/docs/std/collections/string/string_slice/StringSlice/) pointing
...
The external origins, `MutExternalOrigin` and `ImmutExternalOrigin` represent
...
[`alloc()`](/docs/std/memory/unsafe_pointer/alloc/) function returns an
`UnsafePointer` to a new dynamically-allocated block of memory, with the origin
`MutExternalOrigin`. The origin indicates that the memory is not managed by the
...
- An [`AddressSpace`](/docs/std/memory/pointer/AddressSpace/) value.
