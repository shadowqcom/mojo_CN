...
A [`LayoutTensor`](/docs/layout/layout_tensor/LayoutTensor/)
...
- A [layout](/docs/manual/layout/layouts/), defining how the elements are
  laid out in memory.
...
[Introduction to layouts](/docs/manual/layout/layouts/#modes). These layouts
...
Note that this example uses [`memset()`](/docs/std/memory/memory/memset/)
...
[`DeviceBuffer`](/docs/std/gpu/host/device_context/DeviceBuffer/). You can
...
- On the GPU: When a `DeviceBuffer` is passed in to
  `enqueue_function()`, the kernel receives a corresponding
  `UnsafePointer` in place of the `DeviceBuffer.` The kernel can then create a
  `LayoutTensor` using the `pointer`.
...
The
[`LayoutTensor.tile()`](/docs/layout/layout_tensor/LayoutTensor/#tile)
...
The [`LayoutTensorIter`](/docs/layout/layout_tensor/LayoutTensorIter/)
...
[`tiled_iterator()`](/docs/layout/layout_tensor/LayoutTensor/#tiled_iterator)
...
The [`vectorize()`](/docs/layout/layout_tensor/LayoutTensor/#vectorize)
...
- [`copy_from()`](/docs/layout/layout_tensor/LayoutTensor/#copy_from)
  copies data from a source tensor to the current tensor, which may be in a
  different memory space.
- [`copy_from_async()`](/docs/layout/layout_tensor/LayoutTensor/#copy_from_async)
...
The [`layout_tensor` package](/docs/layout/layout_tensor/) also includes
