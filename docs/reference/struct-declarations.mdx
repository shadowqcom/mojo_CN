...
With generics, the parameter's bounds must include `ImplicitlyDestructible`
for this to apply:
...
Without `ImplicitlyDestructible` in the bound, the compiler
can't verify that the struct is safe to destroy implicitly:
...
time. While it often uses trait bounds, it is not limited to traits. A
condition can use any compile-time value that can be evaluated in a clear
and consistent way.
