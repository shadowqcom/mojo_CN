...
- Added new `*_` syntax that allows users to explicitly unbind any number of
  ...
  [partially-bound types](/docs/manual/parameters/#fully-bound-partially-bound-and-unbound-types)
  and
...
- Added
  [`AnyPointer.move_into()`](/docs/std/memory/unsafe_pointer/UnsafePointer/#init_pointee_move)
...
- The [`find()`](/docs/std/builtin/string_literal/StringLiteral/#find),
  ...
  [`count()`](/docs/std/collections/string/string_slice/StringSlice/#count), and
  `__contains__()`
...
- Trivial types, like MLIR types and function types, can now be bound implicitly
  ...
  [`Copyable`](/docs/std/builtin/value/Copyable/), and
  `CollectionElement`.
...
- `vectorize_unroll` has been removed, and
  [`vectorize`](/docs/std/algorithm/functional/vectorize/) now has a parameter
