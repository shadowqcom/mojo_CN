...
- The new [Reflection](/docs/manual/reflection/) page
...
- The Mojo language basic trait hierarchy has changed to expand first-class
  ...
  The [`AnyType`](/docs/std/builtin/anytype/AnyType/),
  [`Movable`](/docs/std/builtin/value/Movable/), and
  [`Copyable`](/docs/std/builtin/value/Copyable/) traits no longer require that
  a type provide a `__del__()` method that may be called by the compiler
...
- `List` slicing without a stride now returns a
  [`Span`](/docs/std/memory/span/Span/), instead of a `List` and no longer
  allocates memory.

- [`InlineArray`](/docs/std/collections/inline_array/InlineArray/) no longer
  conforms to
  [`ImplicitlyCopyable`](/docs/std/builtin/value/ImplicitlyCopyable/). Users
  must explicitly copy arrays or take references.

- [`IndexList`](/docs/std/utils/index/IndexList/) is no longer implicitly
...
- The `strip()`, `lstrip()`, and `rstrip()` methods of `String` and
  [`StringSlice`](/docs/std/collections/string/string_slice/StringSlice/) now
...
- [`OwnedDLHandle.get_symbol()`](/docs/std/ffi/OwnedDLHandle/#get_symbol)
  now returns [`UnsafePointer`](/docs/std/memory/unsafe_pointer/UnsafePointer/)
...
- [`alloc()`](/docs/std/memory/unsafe_pointer/alloc/) now has a
...
- We have removed
  [`Identifiable`](/docs/std/builtin/identifiable/Identifiable/) from enum-like
  types (such as [`DType`](/docs/std/builtin/dtype/DType/) and
  [`AddressSpace`](/docs/std/memory/pointer/AddressSpace/)). This change is
...
- [`Layout`](/docs/layout/layout/Layout/) no longer conforms to
  [`ImplicitlyCopyable`](/docs/std/builtin/value/ImplicitlyCopyable/). The
...
- Docstring validation with `--validate-doc-strings` now emits an error when an
  ...
  [`Raises` docstring](https://github.com/modular/modular/blob/main/mojo/stdlib/docs/docstring-style-guide.md#errors)
...
- `NDBuffer` has been removed. Use
  [`LayoutTensor`](/docs/layout/layout_tensor/LayoutTensor/) instead.
