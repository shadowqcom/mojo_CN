...
- A new, improved
  [`UnsafePointer`](/docs/std/memory/unsafe_pointer/UnsafePointer/) type has
...
- Added a new document on
  [GPU block and warp operations and synchronization](/docs/manual/gpu/block-and-warp/).
...
- `UnsafePointer` has been renamed to `LegacyUnsafePointer` and a new
  [`UnsafePointer`](/docs/std/memory/unsafe_pointer/UnsafePointer/) has
  ...
  *Note*: `LegacyUnsafePointer` and `LegacyOpaquePointer` will eventually be
  ...
  Lastly, [`alloc()`](/docs/std/memory/unsafe_pointer/alloc/) has been moved
...
- `origin_cast()` for
  [`LayoutTensor`](/docs/layout/layout_tensor/LayoutTensor/),
  `NDBuffer` and `UnsafePointer` has
  been deprecated and removed. `LayoutTensor` and `NDBuffer` now support a safer
  `as_any_origin()` origin casting method. `UnsafePointer` has the same safe
  alternative and in addition, it has a safe `as_immutable()`
  casting function and explicitly unsafe `unsafe_mut_cast()` and
  `unsafe_origin_cast()` casting methods.
...
- [`memcpy()`](/docs/std/memory/memory/memcpy/) and
  [`parallel_memcpy()`](/docs/std/algorithm/memory/parallel_memcpy/) without
...
- [`gpu.sync.syncwarp()`](/docs/std/gpu/sync/sync/syncwarp/) now supports
  ...
  [`barrier()`](/docs/std/gpu/sync/sync/barrier/) instead. Note that lane
...
- [`gpu.intrinsics.store_release()`](/docs/std/gpu/intrinsics/store_release/)
  and
  [`gpu.intrinsics.load_acquire()`](/docs/std/gpu/intrinsics/load_acquire/)
...
- The `_GPUAddressSpace` type has been removed and consolidated into
  [`AddressSpace`](/docs/std/memory/pointer/AddressSpace/). GPU-specific
...
- TMA (Tensor Memory Accelerator) types have been moved to a dedicated module.
  ...
  now available from [`gpu.host.nvidia.tma`](/docs/std/gpu/host/nvidia/tma/)
...
- This release includes a number of improvements to elaboration errors. The
  *elaborator* is the compiler pass where final parameter values are determined
  ...
  [`constrained()`](/docs/std/builtin/constrained/constrained/) function), so
