...
- Mojo now supports a standalone `assert` statement, similar to Python's
  ...
  `String`, [`Int`](/docs/std/builtin/int/Int/), etc.). Under
...
- As part of init unification, the `__moveinit__is_trivial` and
  ...
  [`Copyable`](/docs/std/builtin/value/Copyable/) have been
  renamed
...
- Homogeneous variadic parameters are now passed with the
  [`VariadicParamList`](/docs/std/builtin/variadics/VariadicParamList/)
...
- [`String.ljust()`](/docs/std/collections/string/string/String/#ljust),
...
- [`Int.__truediv__()`](/docs/std/builtin/int/Int/#__truediv__)
...
- [`Int.write_padded()`](/docs/std/builtin/int/Int/#write_padded)
...
- The [`Origin`](/docs/std/builtin/type_aliases/Origin/)
...
- [`Span`](/docs/std/memory/span/Span/)`[T]` is no longer
...
- [`TileTensor`](/docs/layout/tile_tensor/TileTensor/) now supports
  hierarchical indexing. For example, a `TileTensor` with shape `(4, (3, 2))`
  can be indexed by `(1)`, `(1, 1)`, or `(1, (1, 1))`.
...
- The [`DimList`](/docs/kernels/buffer/dimlist/DimList/) type has moved to
  ...
  Change `DimList(x, y)` to `DimList[x, y]()`.
...
- [`StringSlice.find`](/docs/std/collections/string/string_slice/StringSlice/#find):
